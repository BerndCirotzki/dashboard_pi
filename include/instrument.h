...
enum DASH_CAP {
  ...
  OCPN_DBP_STC_STW_SOG,    // Speed through Water + SOG
  // Engine Dashboard
  OCPN_DBP_STC_MAIN_ENGINE_RPM,
  ...
  //OCPN_DBP_STC_RSA, double define from Engine Dashboard
  ...
  OCPN_DBP_STC_TANK_LEVEL_WATER_GAUGE_05,
...
public:
  ...
  CapType GetCapacity() { return m_cap_flag; }; // must be defined here!!! It not working when in .ccp file more then 71 !!! Why ??
...
{
public:
    DashboardInstrument_Block(wxWindow* pparent, wxWindowID id, wxString title, InstrumentProperties* Properties, DASH_CAP cap, wxString format);
